...
### Building API and UI from this repository

To replace pre-built images with **local source** (and keep existing Compose volumes), follow the [DEPLOY_GUIDE.md](https://github.com/dograh-hq/dograh/blob/main/DEPLOY_GUIDE.md) in the repo: copy `docker-compose.override.example.yaml` to `docker-compose.override.yaml`, set `COMPOSE_PROJECT_NAME`, initialize the `pipecat` submodule, then `docker compose build api ui` and `docker compose up -d`.

For **Ollama**, **LM Studio**, and custom voices when the API runs in containers, see [Local models with Docker](/configurations/local-models-docker).
