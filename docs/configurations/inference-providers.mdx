...
If you run the stack in **Docker** and want **Ollama**, **LM Studio**, or self-hosted TTS/STT on the same machine, read [Local models with Docker](local-models-docker) for `base_url` and `host.docker.internal`.
